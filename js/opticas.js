...
function guardar_optica(){
	...
	let cat_descuento=$("#cat_descuento").val();
	
	if(nom_optica !="" && num_optica !=""){
		$.ajax({
			url:"../ajax/opticas.php?op=guardar_optica",
			method:"POST",
			data:{nom_optica:nom_optica,num_optica:num_optica,id_usuario:id_usuario,cat_descuento:cat_descuento},
			cache: false,
			dataType: "json",
...
function listar_municipios() {
  ...
  document.getElementById("mun_sucursal").innerHTML += "<option value='"+municipios[i]+"'>"+municipios[i]+"</option>"; 
	}
}
var municipios = [];
...
function guardar_sucursal(){
	...
	let municipio=$("#mun_sucursal").val();
	
	if (nom_sucursal !="" && direccion !="" && telefono!="" && correo !="" && encargado !="" && departamento !="" && municipio !="" ){
	
		$.ajax({
			url:"../ajax/opticas.php?op=guardar_sucursal",
			method:"POST",
			data:{nom_sucursal:nom_sucursal,direccion:direccion,telefono:telefono,correo:correo,encargado:encargado,usuario:usuario,codigo:codigo,id_optica:id_optica,id_sucursal:id_sucursal,departamento:departamento,municipio:municipio},
			cache: false,
			dataType: "json",
			error:function(x,y,z){
				d_pacole.log(x);
